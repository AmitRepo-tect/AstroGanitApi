...
public class DailyHoroscopeController {
	...
	public ResponseEntity<List<DailyHoroscopeAspect>> getDailyRashifal1(@PathVariable("day") String day,
			@PathVariable("langCode") String langCode) {
		LocalDate myDateObj = LocalDate.now().plusDays((long) Integer.parseInt(day));
		DateTimeFormatter myFormatObj = DateTimeFormatter.ofPattern("ddMMyyyy");
		String formattedDate = myDateObj.format(myFormatObj);
	...
	/*
	 * @GetMapping({"/yearlyHoroscope/{langCode}"}) public
	 * ResponseEntity<List<YearlyHoroscopeAspect>>
	 * getYearlyHoroscope(@PathVariable("langCode") String langCode) { String cYear
	 * = "2023"; List<YearlyHoroscopeAspect> yearlyHoroscope =
	 * this.yearlyService.getYearlyHoroscope(cYear); return yearlyHoroscope == null
	 * ? ResponseEntity.status(HttpStatus.NOT_FOUND).build() :
	 * ResponseEntity.status(HttpStatus.OK).body(yearlyHoroscope); }
	 */

	@GetMapping({ "/yearlyHoroscope24/{langCode}" })
	public ResponseEntity<List<YearlyHoroscopeAspect24>> getYearlyHoroscope24(
			@PathVariable("langCode") String langCode) {
		String cYear = "2024";
